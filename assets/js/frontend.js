...
( function () {
	...
	function buildFilters() {
		...
		sorted.forEach( ( [ slug ] ) => {
			...
			const label = taxonomyLabels[ slug ] || slug
				.replace( /^pa_/, '' )
				.replace( /[-_]/g, ' ' )
			...
			window.jQuery( select ).on( 'change', function () {
				...
				if ( slug === 'product_cat' ) {
					...
					const categoryBase  = wcAsyncFilters.category_base || '/product-category/';
					...
					const params = new URLSearchParams();
					Object.entries( currentFilters ).forEach( function ( [ t, v ] ) {
						if ( v && t !== 'product_cat' ) params.set( t, v );
					} );
					const query  = params.toString();
	...
	function fetchProducts( filters, page, pushState ) {
		...
		Object.keys( filters ).forEach( ( taxonomy ) => {
			// product_cat viene gestita come category_path, non in filters[].
			// Inviarla in entrambi i posti creerebbe un duplicato nella tax_query PHP.
			if ( taxonomy !== 'product_cat' ) {
				// La notazione filters[pa_brand]=nike viene interpretata da PHP
				// come $_POST['filters']['pa_brand'] = 'nike'.
				params.set( 'filters[' + taxonomy + ']', filters[ taxonomy ] );
			}
		} );
		...
		fetch( wcAsyncFilters.ajax_url, {
			...
			.then( ( data ) => {
				...
				if ( ! data.success ) {
					if ( grid ) {
						grid.innerHTML = '<p class="wcaf-no-products">' + wcAsyncFilters.i18n.no_products + '</p>';
					}
					return;
				}
				...
				if ( grid ) {
					if ( page === 1 ) {
						...
						const productsWrapper = grid.querySelector( 'ul.products, div.products' );
						if ( productsWrapper ) {
							productsWrapper.insertAdjacentHTML( 'beforeend', result.html );
						} else {
	...
	function updateFilterVisibility( presentTaxonomies ) {
		document.querySelectorAll( '.wcaf-filter-item' ).forEach( ( item ) => {
			...
			item.style.display = presentTaxonomies.includes( taxonomy ) ? 'block' : 'none';
		} );
	}
	 ...
	 *    (es. ?pa_brand=nike) → dobbiamo riapplicare quei filtri.
	 ...
	 * - Parametri GET (?pa_brand=nike) per le tassonomie normali.
	 ...
	 * Quando la pagina si carica con ?pa_brand=nike nell'URL, Select2 non sa
	...
	function restoreSelectValues( filters ) {
		...
		document.querySelectorAll( '.wcaf-filter-select' ).forEach( ( select ) => {
			...
			.then( ( data ) => {
				...
				window.jQuery( select ).append( option ).trigger( 'change.select2' );
			} )
			.catch( () => {
	...
	window.addEventListener( 'popstate', function ( event ) {
		const filters = event.state && event.state.filters ? event.state.filters : {};
