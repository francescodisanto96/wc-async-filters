...
class Admin
{
	...
	public function sanitize_options($raw_input): array
	{
		...
		foreach ($raw_input as $key => $value) {
			...
			// Salviamo '1' come stringa invece di true come booleano.
			// Le stringhe non vuote sono sempre gestite correttamente
			// da WordPress nelle opzioni del database.
			$clean[$key]['enabled'] = (isset($value['enabled']) && $value['enabled'] === '1') ? '1' : '0';
				...
				<table class="wp-list-table widefat fixed striped">
					...
					<tbody>
						<?php foreach ($taxonomies as $slug => $taxonomy) :
							// Confronto esplicito con '1': funziona sia al primo
							// salvataggio che ai successivi, indipendentemente
							// da come WordPress serializza i valori nel database.
							$is_enabled = isset($saved[$slug]['enabled']) && $saved[$slug]['enabled'] === '1';
